...
class ResNeXt(nn.Module):
    ...
    """
    def __init__(self, baseWidth, cardinality, layers, num_classes, in_channel=3):
        """ Constructor
        Args:
        ...
        # in_channel = 3 if not cfg.selector_concat_normal else 6
        # in_channel = 3
        ...
        self.fc = nn.Linear(512 * block.expansion * (int(cfg.ratio / 0.5) ** 2), num_classes)
...
def resnext152(baseWidth = 4, cardinality = 32, num_classes = 1000, in_channel = 3):
    """
    Construct ResNeXt-152.
    """
    model = ResNeXt(baseWidth, cardinality, [3, 8, 36, 3], num_classes, in_channel = in_channel)
    return model
