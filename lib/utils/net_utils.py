...
def load_model(net,
    ...
    print('loaded model: {}'.format(os.path.join(model_dir, '{}.pth'.format(pth))))
...
def load_network(net, model_dir, resume=True, epoch=-1, strict=True):
    ...
    try:
        ...
        if epoch == -1:
            print("Fallback to last model")
            return load_network(net, model_dir, resume, epoch-1)
        else:
            raise ValueError('model not complete')
